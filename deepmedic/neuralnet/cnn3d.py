...
class Cnn3d(object):
    ...
    def _setup_inp_plchldrs(self, train_val_test, inp_shapes_per_path): # TODO: REMOVE for eager
        ...
        if train_val_test == 'train':
            enable_semi_loss_ph = tf.compat.v1.placeholder(
                dtype=tf.bool, shape=[], name='enable_semi_loss'
            )
            lambda_cons_ph = tf.compat.v1.placeholder(
                dtype=tf.float32, shape=[], name='lambda_consistency'
            )
            self._feeds_main['train']['enable_semi_loss'] = enable_semi_loss_ph
            self._feeds_main['train']['lambda_consistency'] = lambda_cons_ph
        return inp_plchldrs
