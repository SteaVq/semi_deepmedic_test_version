...
def process_in_batches(log,
    ...
    n_labeled = batchsize // 2  # Assuming half labeled, half unlabeled in semi-supervised training
    ...
    # Calculate the number of labeled samples (first half) and unlabeled samples (second half)
    ...
    for batch_i in range(n_batches):
        ...
        if train_or_val == "train":
            ...
            min_idx_batch = batch_i * n_labeled
            max_idx_batch = (batch_i + 1) * n_labeled
            enable_semi_loss=max_idx_batch>(len(channs_samples_per_path[0])/2)
            feeds = cnn3d.get_main_feeds('train')
            
            # Get original data for the full batch
            # Get original data for the full batch
            original_x = channs_samples_per_path[0][min_idx_batch: max_idx_batch]
            
            # --- 改进的扰动策略 1: 随机对比度和亮度 ---
            # 假设 original_x shape: [Batch, Channel, D, H, W]
            b_size = original_x.shape[0]
            
            # 1. 随机对比度 (Scale): 0.8 ~ 1.2
            alpha = np.random.uniform(0.8, 1.2, size=(b_size, 1, 1, 1, 1)).astype(original_x.dtype)
            # 2. 随机亮度 (Shift): -0.1 ~ 0.1 (假设数据已归一化到均值0附近)
            beta = np.random.uniform(-0.1, 0.1, size=(b_size, 1, 1, 1, 1)).astype(original_x.dtype)
            
            # 应用变换
            original_x_noisy = original_x * alpha + beta
            
            # 叠加少量高斯噪声以增加纹理扰动 (可选)
            noise = np.random.normal(0, 0.05 * np.std(original_x), original_x.shape).astype(original_x.dtype)
            original_x_noisy = original_x_noisy + noise
            # Combine labeled (original) and unlabeled (noisy) parts
            x_combined = np.concatenate([original_x,original_x_noisy], axis=0)
            
            # Create feed dictionary with the combined input
            feeds_dict = {feeds['x']: x_combined}
            
            # Process each sub-path similarly
            for subs_path_i in range(cnn3d.numSubsPaths):
                original_x_sub = channs_samples_per_path[subs_path_i + 1][min_idx_batch: max_idx_batch]
                
                # Add noise to the unlabeled part
                b_size_sub = original_x_sub.shape[0]
                
                # 1. 随机对比度 (Scale): 0.8 ~ 1.2
                alpha_sub = np.random.uniform(0.8, 1.2, size=(b_size_sub, 1, 1, 1, 1)).astype(original_x_sub.dtype)
                # 2. 随机亮度 (Shift): -0.1 ~ 0.1
                beta_sub = np.random.uniform(-0.1, 0.1, size=(b_size_sub, 1, 1, 1, 1)).astype(original_x_sub.dtype)
                
                # 应用变换
                original_x_sub_noisy = original_x_sub * alpha_sub + beta_sub
                
                # 叠加少量高斯噪声
                noise_sub = np.random.normal(0, 0.05 * np.std(original_x_sub), original_x_sub.shape).astype(original_x_sub.dtype)
                original_x_sub_noisy = original_x_sub_noisy + noise_sub
                
                # Combine labeled (original) and unlabeled (noisy) parts
                x_sub_combined = np.concatenate([original_x_sub, original_x_sub_noisy], axis=0)
                
                feeds_dict.update({feeds['x_sub_' + str(subs_path_i)]: x_sub_combined})
            
            # Use only the labels for the labeled samples (first half)
            lbls_labeled = lbls_samples_per_path[min_idx_batch: max_idx_batch]
            # Combine the labels to match the input batch size
            lbls_batch = np.concatenate([lbls_labeled, lbls_labeled], axis=0)
            feeds_dict.update({feeds['y_gt']: lbls_batch})
            feeds_dict[feeds['enable_semi_loss']] = bool(enable_semi_loss)
            if n_eps_trained_model<4:
                lam = 0.0
            else:
                t = (n_eps_trained_model-3) / (n_epochs-4)  # 归一化到 [0,1]
                lam = 0.2 * np.exp(-5 * (t - 0.5)**2)  # 峰值在 t=0.5
            feeds_dict.update({feeds['lambda_consistency']: np.float32(lam)})
        ...
        # list_RpRnPpPn_per_class holds Real Pos, Real Neg, True Predicted Pos, True Predicted Neg ...
...
def do_training(sessionTf,
    ...
    if num_labeled_samples is not None and num_labeled_samples > 0:
        n_total = len(paths_per_chan_per_subj_train)
        num_labeled_subjs = min(num_labeled_samples, n_total)

        # --- Labeled subset ---
        labeled_paths_per_chan = paths_per_chan_per_subj_train[:num_labeled_subjs]
        labeled_paths_to_lbls = paths_to_lbls_per_subj_train[:num_labeled_subjs]
        if paths_to_masks_per_subj_train is not None:
            labeled_paths_to_masks = paths_to_masks_per_subj_train[:num_labeled_subjs]
        else:
            labeled_paths_to_masks = None
        if paths_to_wmaps_per_sampl_cat_per_subj_train is not None:
            labeled_paths_to_wmaps = paths_to_wmaps_per_sampl_cat_per_subj_train[:num_labeled_subjs]
        else:
            labeled_paths_to_wmaps = None

        # --- Unlabeled subset ---
        unlabeled_paths_per_chan = paths_per_chan_per_subj_train[num_labeled_subjs:]
        unlabeled_paths_to_lbls = paths_to_lbls_per_subj_train[num_labeled_subjs:]  # 占位，会被丢弃
        if paths_to_masks_per_subj_train is not None:
            unlabeled_paths_to_masks = paths_to_masks_per_subj_train[num_labeled_subjs:]
        else:
            unlabeled_paths_to_masks = None
        if paths_to_wmaps_per_sampl_cat_per_subj_train is not None:
            unlabeled_paths_to_wmaps = paths_to_wmaps_per_sampl_cat_per_subj_train[num_labeled_subjs:]
        else:
            unlabeled_paths_to_wmaps = None

        # 如果无标签部分为空，复制一份 labeled 数据避免出错（或跳过）
        if len(unlabeled_paths_per_chan) == 0:
            log.print3("WARNING: No unlabeled subjects. Duplicating labeled data for unlabeled stream.")
            unlabeled_paths_per_chan = labeled_paths_per_chan
            unlabeled_paths_to_lbls = labeled_paths_to_lbls
            unlabeled_paths_to_masks = labeled_paths_to_masks
            unlabeled_paths_to_wmaps = labeled_paths_to_wmaps
    ...
    args_for_sampling_tr = (log,
                            ...
                            n_samples_per_subep_train // 2,
                            sampling_type_inst_tr,
                            inp_shapes_per_path_train,
                            cnn3d.calc_outp_dims_given_inp(inp_shapes_per_path_train[0]),
                            cnn3d.calc_unpredicted_margin(inp_shapes_per_path_train[0]),
                            labeled_paths_per_chan,
                            labeled_paths_to_lbls,
                            labeled_paths_to_masks,
                            labeled_paths_to_wmaps,
                            pad_input,
                            norm_prms,
                            augm_img_prms,
                            augm_sample_prms
                            )
    args_for_sampling_tr_unlabeled = (log,
                                      "train",
                                      num_parallel_proc_sampling,
                                      run_input_checks,
                                      cnn3dWrapper,
                                      ...
                                      cnn3d.calc_outp_dims_given_inp(inp_shapes_per_path_train[0]),
                                      cnn3d.calc_unpredicted_margin(inp_shapes_per_path_train[0]),
                                      unlabeled_paths_per_chan,
                                      unlabeled_paths_to_lbls,
                                      unlabeled_paths_to_masks,
                                      unlabeled_paths_to_wmaps,
                                      pad_input,
                                      norm_prms,
                                      augm_img_prms,
                                      augm_sample_prms
    ...
    sampling_job_submitted_train_unlabel = False
            ...
            for subep in range(n_subepochs):
                ...
                if val_on_samples:
                    ...
                    if mp_pool is not None:
                        ...
                        sampling_job_tr_unlabel = mp_pool.apply_async(get_samples_for_subepoch,
                                                                      args_for_sampling_tr_unlabeled)
                        sampling_job_submitted_train_unlabel = True
                ...
                if mp_pool is None:  # Sequential processing.
                    ...
                    (channs_samples_per_path_tr,
                     lbls_samples_per_path_tr) = sampling_job_tr.get()
                    sampling_job_submitted_train = False
                # ----------------------- GET DATA FOR THIS SUBEPOCH's TRAINING unlabel------------------------------
                if mp_pool is None:  # Sequential processing.
                    (channs_samples_per_path_tr_unlabel,
                     lbls_samples_per_path_tr_unlabel) = get_samples_for_subepoch(*args_for_sampling_tr_unlabeled)
                elif sampling_job_submitted_train_unlabel:  # done parallel with train/val of previous epoch.
                    (channs_samples_per_path_tr_unlabel,
                     lbls_samples_per_path_tr_unlabel) = sampling_job_tr_unlabel.get()
                    sampling_job_submitted_train_unlabel = False
                else:  # Not previously submitted in case of first epoch or after a full-volumes validation.
                    assert subep == 0
                    sampling_job_tr_unlabel = mp_pool.apply_async(get_samples_for_subepoch,
                ...
                # ----------------------- COMBINE LABELED AND UNLABELED DATA ------------------------------
                    # 直接将有标签数据放在前面，无标签数据放在后面

                # Combine data for training - simply concatenate labeled and unlabeled data
                channs_samples_per_path_combined = []
                for path_idx in range(len(channs_samples_per_path_tr)):
                    # Concatenate labeled and unlabeled samples directly
                    combined_path_data = np.concatenate([
                        channs_samples_per_path_tr[path_idx], 
                        channs_samples_per_path_tr_unlabel[path_idx]
                    ], axis=0)
                    channs_samples_per_path_combined.append(combined_path_data)

                # Combine labels - concatenate labeled and unlabeled labels directly
                lbls_samples_per_path_combined = np.concatenate([
                    lbls_samples_per_path_tr, 
                    lbls_samples_per_path_tr_unlabel
                ], axis=0)
                ...
                # Calc num of batches from extracted samples, in case not extracted as much as requested.
                n_batches_train_total = len(channs_samples_per_path_combined[0]) // batchsize_train
                process_in_batches(log,
                                   sessionTf,
                                   "train",
                                   n_batches_train_total,
                                   batchsize_train,
                                   cnn3d,
                                   acc_monitor_ep_tr,
                                   channs_samples_per_path_combined,
                                   lbls_samples_per_path_combined,
                                   n_eps_trained_model,
                                   n_epochs)
